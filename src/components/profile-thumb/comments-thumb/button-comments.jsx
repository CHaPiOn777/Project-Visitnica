...
const ButtonComments = ({ user, block } ) => {
  ...
  let asd;
  if (block) {
    asd = comments?.items.filter(item => item.target === block || null)
  } else {
    asd = commentCount
  }
  
  const commentAccessMode = user.curator ? 'admin' : user.owner ? 'owner' : 'admin';
  return (
    <>
      <button className={styles['chat-icon']} onClick={toggleComments} type='button'>
        <div className={styles['comment-counter']}>
          { asd?.length ?
          (asd.length < 100 ? asd.length : '99+') : commentCount < 100 ? commentCount : '99+'}
        </div>
      ...
      <div className={styles['comment-container']}>
        {commentVisible && <Comment target={user._id} rules={'admin'} />}
      </div>
    </>
  );
};
