...
export default function ProfileThumb (props: TThumbProps) {
  ...
  const commentAccessMode = props.curator ? 'admin' : props.owner ? 'owner' : 'user';

  return (
    <article className={styles.container}>
      <button className={styles['chat-icon']} onClick={toggleComments}>
        {(props.owner || props.curator) && <div className={styles['comment-counter']}>
          {commentCount < 100 ? commentCount : '99+'}
        </div>}
      ...
      <div className={styles['comment-container']}>
        {commentVisible && <Comment target={props.id} access={commentAccessMode} />}
      </div>
      <Link className={styles['link-overlay']} to="/anothertmp">
